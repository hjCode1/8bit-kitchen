...
export default function IngredientChecklist({ requiredIngredients, fridgeIngredientNames }: IngredientChecklistProps) {
  return (
    <div className="bg-pixel-light border-4 border-pixel-border p-4 mb-4">
      <h3 className="font-pixel text-[17px] text-pixel-text mb-3">📋 필요한 재료</h3>
        ...
        {requiredIngredients.map((name) => {
          ...
          return (
            <li key={name} className="flex items-center gap-2">
              <span className={`font-pixel text-[16px] ${inFridge ? 'text-pixel-green' : 'text-pixel-red'}`}>
                {inFridge ? '✅' : '❌'}
              </span>
              <span className={`font-pixel text-[16px] ${inFridge ? 'text-pixel-text' : 'text-pixel-text/50'}`}>
                {name}
              </span>
              {inFridge && (
                <span className="font-pixel text-[15px] text-pixel-green">(보유)</span>
              )}
            </li>
          );
        })}
